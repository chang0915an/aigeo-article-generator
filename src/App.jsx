...
export default function App() {
  ...
  const [currentStep, setCurrentStep] = useState(0); // 0: Idle, 1: Step1, 2: Step2, 3: Step3
  ...
  const handleGenerate = async (formData) => {
    ...
    try {
      ...
      if (!response.ok || !resJson.success) {
        throw new Error(resJson.error || '生成失败，请检查后端网络与 API Key 配置');
      }
          ...
          <div className="flex items-start space-x-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1 leading-relaxed">
                只需输入长尾词、品牌产品元素与主题方向，系统将先使用<strong className="text-cyan-300 font-normal">豆包/火山方舟联网 API</strong> 检索第一名竞品的推荐逻辑，再结合您真实的品牌元素生成携程笔记，最后二次联网校验设施与事实。
              </p>
            </div>
          </div>
        </div>
              ...
              <div className="bg-rose-950/50 border border-rose-800/80 rounded-2xl p-5 text-rose-200 text-xs flex items-start space-x-3 backdrop-blur animate-shake">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-rose-400/80 mt-2">
                    💡 提示：在 Netlify 后台的 <code>Site configuration &gt; Environment variables</code> 中添加 <code>VOLC_API_KEY</code>，或在表单“高级 API 配置”中填写您的 Key 即可。
                  </p>
                </div>
              </div>
            )}
